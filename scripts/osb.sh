...
if [[ ${OSB_CHOICE} == 'true' ]]; then
    ...
    aws s3 cp s3://${INSTALLER_S3_BUCKET}/osbws.zip /u01/install/osbws.zip
    cd /u01/install/
    unzip osbws.zip
    ...
    echo OSB_WS_BUCKET=${OSB_AWS_BUCKET}  >>/u01/app/oracle/product/12.1.0.2/db_1/dbs/osbws${PRIMARY_NAME}.ora
